...
import { migrateLegacyPreferences } from "@/lib/migrateLocalStorageToDb";
...
export default function AppShell({
  ...
  useEffect(() => {
    ...
    migrateLegacyPreferences(supabase, userId, userPreferences).catch(() => {});
  ...
  return (
    <div className="min-h-screen bg-bg">
      ...
      <aside className="hidden md:flex md:w-64 md:flex-col md:fixed md:inset-y-0 bg-bg-elevated border-r border-bg-border z-30">
        ...
        <SidebarFooter
          userEmail={userEmail}
          activeAccountIdProp={userPreferences.active_account_id}
        ...
        <header className="md:hidden sticky top-0 z-20 bg-bg-elevated/80 backdrop-blur-xl border-b border-bg-border safe-area-top">
          <div className="px-4 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Logo size="sm" />
              <span className="text-white font-semibold tracking-tight">GoldLedger</span>
            </div>
            <div className="w-9" />
          </div>
        </header>
      ...
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-30 bg-bg-elevated/95 backdrop-blur-xl border-t border-bg-border safe-area-bottom">
        <div className="flex items-center justify-around px-4 pt-3 pb-3 gap-4">
          {mobileBottomNav.map((item) => {
            ...
            if (item.primary) {
              return (
                <Link key={item.href} href={item.href} className="flex-1 group active:scale-95 transition-transform">
                  <div className="flex items-center justify-center gap-2 px-4 py-3 rounded-2xl bg-gradient-to-r from-gold-500 to-gold-600 shadow-lg shadow-gold-500/30 group-hover:shadow-gold-500/50 transition-all">
                    ...
                    <span className="text-sm font-semibold text-bg">{item.label}</span>
                  </div>
                </Link>
              );
            }
            return (
              <Link key={item.href} href={item.href} className="flex-1 active:scale-95 transition-transform">
                <div className={cn(
...
function SidebarBranding({ compact }: { compact?: boolean }) {
  return (
    <div className={cn("flex items-center gap-3", compact ? "" : "px-6 py-6 border-b border-bg-border")}>
      <Logo size="md" />
      <div>
        <div className="text-white font-semibold tracking-tight leading-tight">GoldLedger</div>
        <div className="text-[11px] text-zinc-500 leading-tight">Trading Journal</div>
      </div>
    </div>
  );
}
...
function SidebarFooter({
  ...
  return (
    <div className="px-3 py-4 border-t border-bg-border space-y-2">
      ...
      <AccountSwitcher
        userEmail={userEmail}
        activeAccountIdProp={activeAccountIdProp}
      />
